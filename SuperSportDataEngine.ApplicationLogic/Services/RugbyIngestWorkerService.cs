namespace SuperSportDataEngine.ApplicationLogic.Services
{
    ...
    public class RugbyIngestWorkerService : IRugbyIngestWorkerService
    {
        ...
        private async Task IngestLogsHelper(List<RugbyTournament> tournaments, CancellationToken cancellationToken)
        {
            ...
            foreach (var tournament in tournaments)
            {
                ...
                else
                {
                    ...
                    await CleanUpGroupedLogs(cancellationToken, logs);
        ...
        private async Task IngestStandingsForJuniorRugbyWorldCup(
            CancellationToken cancellationToken,
            int groupHierarchyLevel,
            RugbyGroupedLogsResponse logs,
            IEnumerable<Ladderposition> ladderPositions)
        {
            ...
            foreach (var ladder in ladderPositions)
            {
                ...
                {
                    var key = GetType().FullName + ".IngestStandingsForJuniorRugbyWorldCup" + ". Season=" + seasonId;
                    var message = $"Error processing RugbyLogGroup for ladderId={ladder.group}, groupName={ladder.groupName}. Check that the RugbyLogGroup entry is configured in the DB.";
                    await _logger.Error(key, exception, $"{message}{Environment.NewLine}{key}{Environment.NewLine}{exception}");
                }
            }
        ...
        private async Task IngestStandingsForRugbyWorldCup(CancellationToken cancellationToken, int groupHierarchyLevel, RugbyGroupedLogsResponse logs, List<Ladderposition> ladderPositions)
        {
            var tournamentId = logs.RugbyGroupedLogs.competitionId;
            var seasonId = logs.RugbyGroupedLogs.seasonId;
            ...
            foreach (var ladder in ladderPositions)
            {
                ...
                {
                    var key = GetType().FullName + ".IngestStandingsForRugbyWorldCup" + ". Season=" + seasonId;
                    var message = $"Error processing RugbyLogGroup for ladderId={ladder.group}, groupName={ladder.groupName}. Check that the RugbyLogGroup entry is configured in the DB.";
                    await _logger.Error(key, exception, $"{message}{Environment.NewLine}{key}{Environment.NewLine}{exception}");
                }
            }
        ...
        private async Task IngestStandingsForRugbyChallenge2018(
            CancellationToken cancellationToken,
            int groupHierarchyLevel,
            RugbyGroupedLogsResponse logs,
            IEnumerable<Ladderposition> ladderPositions)
        {
            ...
            foreach (var ladder in ladderPositions)
            {
                ...
                {
                    var key = GetType().FullName + ".IngestStandingsForRugbyChallenge" + ". Season=" + seasonId;
                    var message = $"Error processing RugbyLogGroup for ladderId={ladder.group}, groupName={ladder.groupName}. Check that the RugbyLogGroup entry is configured in the DB.";
                    await _logger.Error(key, exception, $"{message}{Environment.NewLine}{key}{Environment.NewLine}{exception}");
                }
            }
        ...
        private async Task IngestOverallStandingsForSevens2019(
            CancellationToken cancellationToken,
            int groupHierarchyLevel,
            RugbyGroupedLogsResponse logs,
            IEnumerable<Ladderposition> ladderPositions)
        {
            ...
            foreach (var ladder in ladderPositions)
            {
                ...
                {
                    var key = GetType().FullName + ".IngestOverallStandingsForSevens2019" + ". Season=" + seasonId;
        ...
        private async Task IngestStandingsForSevens2019(
            CancellationToken cancellationToken,
            int groupHierarchyLevel,
            RugbyGroupedLogsResponse logs,
            IEnumerable<Ladderposition> ladderPositions)
        {
            ...
            foreach (var ladder in ladderPositions)
            {
                ...
                {
                    var key = GetType().FullName + ".IngestStandingsForSevens2019" + ". Season=" + seasonId;
                    var message = $"Error processing RugbyLogGroup for ladderId={ladder.group}, groupName={ladder.groupName} roundNumber={ladder.roundNumber}. Check that the RugbyLogGroup entry is configured in the DB.";
                    await _logger.Warn(
                        key,
                        exception,
        ...
        private async Task PersistGroupedLogs(CancellationToken cancellationToken, RugbyGroupedLogsResponse logs)
        {
            ...
            if (competitionId == RugbyStatsProzoneConstants.ProviderTournamentIdRugbyWorldCup)
            {
                ...
                {
                    await IngestStandingsForRugbyWorldCup(cancellationToken, 1, logs, logs.RugbyGroupedLogs.ladderposition);
                }
            }
            ...
            if (competitionId == RugbyStatsProzoneConstants.ProviderTournamentIdJuniorRugbyWorldCup)
            {
                if (logs.RugbyGroupedLogs.overallStandings == null &&
                    logs.RugbyGroupedLogs.groupStandings == null &&
                    logs.RugbyGroupedLogs.secondaryGroupStandings == null)
                {
                    await IngestStandingsForJuniorRugbyWorldCup(cancellationToken, 1, logs, logs.RugbyGroupedLogs.ladderposition);
                }
            }
            ...
            if (competitionId == RugbyStatsProzoneConstants.ProviderTournamentIdSevensRugby)
            {
                ...
                if (seasonId == RugbyStatsProzoneConstants.ProviderTournamentSeasonId2019)
                {
                    //[TODO] This should be removed when we correctly serve out overall standings data
                    //[TODO] This is dependant on the Apps/Web/SS4 API to handle the new structure

                    //if (logs.RugbyGroupedLogs.overallStandings != null)
                    //    await IngestOverallStandingsForSevens2019(cancellationToken, 1, logs, logs.RugbyGroupedLogs.overallStandings.ladderposition);

                    if (logs.RugbyGroupedLogs.groupStandings != null)
                        await IngestStandingsForSevens2019(cancellationToken, 3, logs, logs.RugbyGroupedLogs.groupStandings.ladderposition);
                }
            }
            ...
            if (competitionId == RugbyStatsProzoneConstants.ProviderTournamentIdSupersportChallenge)
            {
                if (seasonId == RugbyStatsProzoneConstants.ProviderTournamentSeasonId2018)
                {
                    if (logs.RugbyGroupedLogs.ladderposition != null)
                        await IngestStandingsForRugbyChallenge2018(cancellationToken, 1, logs, logs.RugbyGroupedLogs.ladderposition);
                }
            }
        ...
        private async Task CleanUpGroupedLogs(CancellationToken cancellationToken, RugbyGroupedLogsResponse logsFromProvider)
        {
            if (cancellationToken.IsCancellationRequested)
                return;

            await CleanUpOverallStandings(
                logsFromProvider.RugbyGroupedLogs.competitionId,
                logsFromProvider.RugbyGroupedLogs.seasonId, 
                logsFromProvider.RugbyGroupedLogs.overallStandings);

            await CleanUpGroupStandings(
                logsFromProvider.RugbyGroupedLogs.competitionId,
                logsFromProvider.RugbyGroupedLogs.seasonId,
                logsFromProvider.RugbyGroupedLogs.groupStandings);

            await CleanUpSecondaryGroupStandings(
                logsFromProvider.RugbyGroupedLogs.competitionId,
                logsFromProvider.RugbyGroupedLogs.seasonId,
                logsFromProvider.RugbyGroupedLogs.secondaryGroupStandings);
        }

        private async Task CleanUpSecondaryGroupStandings(int competitionId, int seasonId, SecondaryGroupStandings secondaryGroupStandings)
        {
            // All the log entries that we have in the db for this tournament and season.
            var logsInDb = _publicSportDataUnitOfWork.RugbyGroupedLogs.Where(l =>
        ...
        private async Task CleanUpGroupStandings(int competitionId, int seasonId, GroupStandings groupStandings)
        {
            // All the log entries that we have in the db for this tournament and season.
            var logsInDb = _publicSportDataUnitOfWork.RugbyGroupedLogs.Where(l =>
                l.RugbyTournament.ProviderTournamentId == competitionId &&
        ...
        private async Task CleanUpOverallStandings(int competitionId, int seasonId, OverallStandings overallStandings)
        {
            ...
            // For each of those entries, log them as a 
            // warning that we are going to remove them.
            var standingsToRemove = overallStandingsToRemove as IList<RugbyGroupedLog> ?? overallStandingsToRemove.ToList();
            foreach (var rugbyGroupedLog in standingsToRemove)
            {
                await _logger.Warn("CleanUpGroupedLogs." +
                                   rugbyGroupedLog.RugbyTournament.ProviderTournamentId + "." +
